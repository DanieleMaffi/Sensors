...
fetch("https://hf3xzw.deta.dev/")
    ...
    .then(body => {
        ...
        `
            // Sensors display state init
            if (sensor.value) {
                document.getElementById("lightBulb" + i).classList.add('light-on')
                document.getElementById('changeValue' + i).innerHTML = 'On'
            }
            else 
                document.getElementById('changeValue' + i).innerHTML = 'Off'
        ...
        for (let i = 0; i < 2; i++) {
            document.getElementById('changeValue' + i).onclick = () => {
                ...
                toggleButtonState(i)
                fetch('https://hf3xzw.deta.dev/' + JSONToSensor(body['sensors'][i].id) + '/toggle', { method: 'PUT' })
                    .then(response => response.json())
                    .then(result => {
                        console.log("Sensor state changed!")
                    })
                }
        }
